...
import {
  listOutboundUrlSettings,
  updateOutboundUrlSetting,
...
const STATUS_OPTIONS: MateReportStatus[] = [
  "open",
  "reviewing",
  "resolved",
  "dismissed",
...
export function AdminPanel() {
  ...
  const [statusFilter, setStatusFilter] = useState<MateReportStatus | "all">(
    "all",
  );
  ...
  const loadAll = async () => {
    ...
    setLoading(true);
    try {
      const [reportList, settings] = await Promise.all([
        listReports(),
        listOutboundUrlSettings(),
      ]);
                ...
                <div className="flex flex-wrap items-center justify-between gap-sm">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleStatusChange(
                        report.id,
                        e.target.value as MateReportStatus,
                      )
                    }
      ...
      <section>
        ...
        <div className="mt-sm flex flex-col gap-sm">
          {urlSettings.map((setting) => (
            <div
              key={setting.id}
              className="flex flex-col gap-xs sm:flex-row sm:items-center"
            >
