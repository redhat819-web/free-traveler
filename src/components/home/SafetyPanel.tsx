...
export function SafetyPanel({ initialCountry }: { initialCountry?: string }) {
  ...
  const openEntry =
    countrySafety.find((entry) => entry.country === openCountry) ?? null;
    ...
    <section className="bg-bg-soft px-gutter py-xl">
      <div className="mx-auto max-w-container-max">
        ...
        <p className="mt-xs text-sm text-text-secondary">
          외교부 등 공식 출처를 바탕으로 정리한 안전정보이며, 최종 확인일 기준
          7일이 지나면 최신 여부를 다시 확인해야 합니다.
        </p>

        <div className="mt-lg grid grid-cols-1 gap-md sm:grid-cols-2 lg:grid-cols-3">
          {featured.map((entry) => {
            const stale =
              daysSince(entry.source.lastVerified) > STALE_THRESHOLD_DAYS;
            return (
              <button
                key={entry.id}
                type="button"
            ...
            >
              ...
              >
                ...
                <dl className="mt-md grid grid-cols-1 gap-md sm:grid-cols-2">
                  {CATEGORY_LABELS.map(({ key, label }) => (
                    <div key={key}>
                      ...
                      <dd className="mt-2xs text-sm text-text-secondary">
                        {openEntry[key]}
                      </dd>
                    </div>
                  ))}
                ...
                <div className="mt-lg flex flex-wrap items-center justify-between gap-sm border-t border-border-hairline pt-md text-xs text-text-muted">
                  <span>
                    출처: {openEntry.source.name} · 확인일{" "}
                    {openEntry.source.lastVerified} · 작성:{" "}
