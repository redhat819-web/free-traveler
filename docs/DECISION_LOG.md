...
## DEC-015 — 초기 소급 커밋 구간은 개별 빌드 가능성을 보장하지 않음

- **Status:** Accepted
- **Context:** 2026-09-19까지 W02~W22의 코드가 전부 작성되었으나 Git에는 한 번도 커밋되지
  않은 채로 누적되어 있었다(`git remote`에 저장소는 연결되어 있었으나 `push` 이력 없음).
  이 코드를 Wave/Task 단위로 소급 커밋하는 과정에서, `src/lib/mate-state.ts`처럼 일부
  Wave 늦은 Task(W22 `UNIT-MATE-STATE`)의 산출물을 더 이른 Wave(W15
  `CMP-SCR004-LIST` 등)의 컴포넌트가 이미 참조하도록 리팩터링되어 있어, Wave 번호
  오름차순으로 그대로 커밋하면 일부 중간 커밋이 개별적으로는 빌드되지 않는 상황이
  발견되었다.
- **Decision:** 이 최초 소급 커밋 구간(대략 2026-09-19 하루치, 이 결정 이전에 생성된
  로컬 커밋들 포함 — `mate-state.ts` 관련 4개 커밋이 이미 이렇게 생성되어 있었음)은
  **개별 커밋의 단독 빌드 가능성을 보장하지 않는다.** 검증은 **최종 HEAD 기준으로만**
  수행한다(`tsc --noEmit`, `eslint`, `vitest run`, Playwright Chromium E2E).
  `git rebase -i`(대화형) 또는 이를 우회하는 비대화형 재작성(`rebase --onto` 등)으로
  히스토리를 바로잡는 방안은 검토했으나 채택하지 않는다.
- **Rationale:** 이 구간의 32개 이상 커밋은 어느 것도 원격에 push된 적이 없고
  GitHub Actions CI를 통과한 적도 없다 — 즉 "커밋 단위로 안정적으로 개발되어 온
  히스토리"가 애초에 존재하지 않았고, 지금 만드는 것은 이미 완성된 코드를 사후에
  Wave/Task 단위로 정리해 기록하는 소급 커밋이다. 없던 점진적 개발 순서를 사후에
  꾸며내기 위해 소수 파일(`mate-state.ts` 등)만 재배치하면 부분적으로만 정합해지고
  나머지 구간의 근본적 한계(사후 커밋이라는 특성)는 해소되지 않는다. 히스토리 재작성은
  아직 push 전이라 기술적으로는 안전하지만, 부분적 이득을 위해 감수할 위험은 아니라고
  판단했다.
- **Consequence:**
  - 이 구간에서 `git bisect`를 사용하는 것은 신뢰할 수 없다 — 중간 커밋 다수가
    빌드되지 않을 수 있다.
  - 이후(이 결정 이후) 커밋되는 코드는 이 예외 대상이 아니며, 정상적으로 각 커밋이
    최소한 타입체크를 통과하는 상태를 유지한다.
  - 이 구간의 시작을 표시하기 위해 커밋 메시지에 `retroactive:` 접두사를 사용한
    시작 커밋을 둔다(예: `retroactive: begin Wave-ordered catch-up commit of
    pre-existing uncommitted code (see DEC-015)`).
- **관련 문서:** `TASKS/WAVE_STATE.json`, `TASKS/WAVE_PLAN.md`, git 커밋 로그
  (2026-09-19, `retroactive:` 접두사 커밋부터 `RELEASE-CHECK-VERCEL-SUPABASE` 착수
  직전까지)
