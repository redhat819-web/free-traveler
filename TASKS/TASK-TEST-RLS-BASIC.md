...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 57번 행(`TEST-RLS-BASIC`)을 상세화한 것이다.
- Category: **INTEGRATION_TEST** / 우선순위: **P0**
- 선행 Task(DB-RLS-BASE, DB-ACCESS)가 먼저 완료되어야 이 Task를 시작할 수 있다.
...
- `tests/rls/basic.test.ts` (create)

> Expected Files 밖의 파일은 생성·수정하지 않는다.

## Functional AC

- 본인/타인/Moderator/Admin 각 역할의 비공개 데이터 접근 시도가 전부 403 또는 빈 결과로 처리됨을 검증
