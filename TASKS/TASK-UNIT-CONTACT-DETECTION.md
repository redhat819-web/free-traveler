...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 55번 행(`UNIT-CONTACT-DETECTION`)을 상세화한 것이다.
- Category: **UNIT_TEST** / 우선순위: **P0**
- 선행 Task(CMP-SCR003-MATE-COMPOSE)가 먼저 완료되어야 이 Task를 시작할 수 있다.
...
- `src/lib/contact-detection.test.ts` (create)

> Expected Files 밖의 파일은 생성·수정하지 않는다.

## Functional AC

- 전화번호/이메일/메신저 ID 패턴 기준 테스트셋 탐지율 95%+, 오탐 5% 이하
