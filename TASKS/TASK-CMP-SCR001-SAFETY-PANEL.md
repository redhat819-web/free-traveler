# CMP-SCR001-SAFETY-PANEL — 국가 안전정보 패널

- **Task ID:** `CMP-SCR001-SAFETY-PANEL`
- **제목:** 국가 안전정보 패널
- **Category:** COMPONENT
- **Type:** component
- **Priority:** P1
...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 9번 행(`CMP-SCR001-SAFETY-PANEL`)을 상세화한 것이다.
- Category: **COMPONENT** / 우선순위: **P1**
- 선행 Task(DATA-SAFETY)가 먼저 완료되어야 이 Task를 시작할 수 있다.
...
- `src/components/home/SafetyPanel.tsx` (create)

> Expected Files 밖의 파일은 생성·수정하지 않는다.

## Functional AC

- 치안·사기·법규·교통·재난·보건·문화·긴급연락처 8개 카테고리, 출처·확인일, 외교부 원문 링크(`noopener,noreferrer`), 중대 경보 텍스트 라벨, 국가/지역 범위 구분

## Visual AC

- 7일 초과 시 stale 배지(색상+텍스트 병행)
