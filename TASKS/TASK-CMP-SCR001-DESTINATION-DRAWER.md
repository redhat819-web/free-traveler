# CMP-SCR001-DESTINATION-DRAWER — 여행지 상세 Drawer(+ 안전정보 전환)

- **Task ID:** `CMP-SCR001-DESTINATION-DRAWER`
- **제목:** 여행지 상세 Drawer(+ 안전정보 전환)
- **Category:** COMPONENT
- **Type:** component
- **Priority:** P0
...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 8번 행(`CMP-SCR001-DESTINATION-DRAWER`)을 상세화한 것이다.
- Category: **COMPONENT** / 우선순위: **P0**
- 선행 Task(DATA-DESTINATIONS, DATA-SAFETY)가 먼저 완료되어야 이 Task를 시작할 수 있다.
...
- `src/components/home/DestinationDrawer.tsx` (create)

> Expected Files 밖의 파일은 생성·수정하지 않는다.

## Functional AC

- 소개·명소 5+·일정·예산·교통·음식 3+·에티켓·출처, 관련 여행지 최대 6개, "국가 안전정보 보기" 내부 전환

## Visual AC

- Desktop 우측 40% 슬라이드/Mobile 하단 풀시트, 그림자는 Drawer에만 적용
