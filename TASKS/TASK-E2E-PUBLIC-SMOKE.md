# E2E-PUBLIC-SMOKE — 공개 흐름 Smoke(홈→상세→안전정보, About, 동행 열람)

- **Task ID:** `E2E-PUBLIC-SMOKE`
- **제목:** 공개 흐름 Smoke(홈→상세→안전정보, About, 동행 열람)
- **Category:** E2E_TEST
- **Type:** test
- **Priority:** P0
...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 58번 행(`E2E-PUBLIC-SMOKE`)을 상세화한 것이다.
- Category: **E2E_TEST** / 우선순위: **P0**
- 선행 Task(PAGE-SCR001, PAGE-SCR002, PAGE-SCR004, GLOBAL-ERROR-PAGES)가 먼저 완료되어야 이 Task를 시작할 수 있다.
...
- (SCR-001/002/004 공개 열람 다수 REQ의 통합 검증)
...
- `tests/e2e/public-smoke.spec.ts` (create)

> Expected Files 밖의 파일은 생성·수정하지 않는다.

## Functional AC

- Playwright **Chromium 프로젝트만**: 홈 진입→국내/해외 탭→여행지 상세 Drawer→안전정보 전환, About 페이지 렌더, 동행 목록 비로그인 열람, 404 라우트 확인
