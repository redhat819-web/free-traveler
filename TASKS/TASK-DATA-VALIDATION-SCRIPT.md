# DATA-VALIDATION-SCRIPT — 정적 데이터 필수 필드 검증 스크립트

- **Task ID:** `DATA-VALIDATION-SCRIPT`
- **제목:** 정적 데이터 필수 필드 검증 스크립트
- **Category:** DATA
- **Type:** static_data
- **Priority:** P1
...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 41번 행(`DATA-VALIDATION-SCRIPT`)을 상세화한 것이다.
- Category: **DATA** / 우선순위: **P1**
- 선행 Task(DATA-DESTINATIONS, DATA-SAFETY, DATA-REPRESENTATIVE)가 먼저 완료되어야 이 Task를 시작할 수 있다.
...
- 여행지/안전/대표 데이터의 필수 필드 누락을 CI/로컬에서 검출, 실패 시 게시 차단
