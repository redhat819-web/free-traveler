...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 28번 행(`CMP-SCR004-LIST`)을 상세화한 것이다.
- Category: **COMPONENT** / 우선순위: **P0**
- 선행 Task(DB-ACCESS, CMP-SCR004-FILTER)가 먼저 완료되어야 이 Task를 시작할 수 있다.
...
- `src/components/mates/MatePostList.tsx` (create)

> Expected Files 밖의 파일은 생성·수정하지 않는다.

## Functional AC

- 최대 8개 우선 노출, 종료일 경과 시 CLOSED로 표시(조회 시점 계산)

## Visual AC

- Card 최대 8개, 그 이상 "더 보기"
