...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 54번 행(`UNIT-TRAVEL-DATES`)을 상세화한 것이다.
- Category: **UNIT_TEST** / 우선순위: **P0**
- 선행 Task(CMP-SCR003-FLIGHT-FORM, CMP-SCR003-HOTEL-FORM)가 먼저 완료되어야 이 Task를 시작할 수 있다.
...
- `src/lib/date-validation.test.ts` (create)

> Expected Files 밖의 파일은 생성·수정하지 않는다.

## Functional AC

- 과거 출발일/체크인, 역전 날짜, 경계값(당일/전날) 케이스 전부 검증
