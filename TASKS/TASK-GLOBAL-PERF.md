# GLOBAL-PERF — 성능 최적화(LCP/INP/CLS/이미지)

- **Task ID:** `GLOBAL-PERF`
- **제목:** 성능 최적화(LCP/INP/CLS/이미지)
- **Category:** GLOBAL
- **Type:** global
- **Priority:** P1
...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 51번 행(`GLOBAL-PERF`)을 상세화한 것이다.
...
- `next.config.ts`, 이미지 사용처 전반 (modify)

> Expected Files 밖의 파일은 생성·수정하지 않는다.

## Functional AC

- `next/image` 반응형+lazy, 서버 컴포넌트 우선

## Visual AC

- 이미지 크기 고정으로 레이아웃 이동 최소화
