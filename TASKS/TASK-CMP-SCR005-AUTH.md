# CMP-SCR005-AUTH — Guest 인증(로그인/가입/재설정/성인확인/탈퇴)

- **Task ID:** `CMP-SCR005-AUTH`
- **제목:** Guest 인증(로그인/가입/재설정/성인확인/탈퇴)
- **Category:** AUTH
- **Type:** auth
- **Priority:** P0
...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 33번 행(`CMP-SCR005-AUTH`)을 상세화한 것이다.
- Category: **AUTH** / 우선순위: **P0**
- 선행 Task(DB-SCHEMA-BASE, DB-RLS-BASE)가 먼저 완료되어야 이 Task를 시작할 수 있다.
...
- 이메일 가입·인증·로그인·로그아웃·재설정, 성인 확인은 `is_adult`/`adult_verified_at`만 저장(생년월일 미저장), 탈퇴 시 즉시 비식별화

## Visual AC

- Guest 3-Card(로그인/가입/재설정) Desktop 가로/Mobile 세로
- Lorem ipsum, '준비 중', '정보 확인 필요' 문구 및 내용 없는 Card를 두지 않는다(D-001/DESIGN.md 공통 규칙).

## Security/Privacy AC

- 미인증 이메일은 동행 쓰기 권한 없음, 삭제 감사 기록
