# MANUAL-CHECK-CLIENT-STATE — 클라이언트 상태 수동 확인(즐겨찾기/공유/Toast)

- **Task ID:** `MANUAL-CHECK-CLIENT-STATE`
- **제목:** 클라이언트 상태 수동 확인(즐겨찾기/공유/Toast)
...
- 이 Task는 `TASKS/00_TASK_LIST.md`의 Seq 64번 행(`MANUAL-CHECK-CLIENT-STATE`)을 상세화한 것이다.
- Category: **MANUAL_CHECK** / 우선순위: **P2**
- 선행 Task(CMP-SCR001-FAVORITES, CMP-SCR001-SHARE, GLOBAL-TOAST)가 먼저 완료되어야 이 Task를 시작할 수 있다.
...
- `localStorage` 즐겨찾기 추가/해제 지속성, Web Share 실패 시 URL 복사 폴백, Toast 자동 소멸·수동 닫기 확인
